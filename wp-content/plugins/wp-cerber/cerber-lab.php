...
function lab_validate_lic( $lic = '', &$msg = '', &$site_ip = '' ) {
	...
	if ( ! $expires ) {
		$msg .= '(4.' . $i . '.' . $cerber_lab_last_node_id . '.' . crb_escape_html( crb_array_get( $ret, array( 'response', 'expires_gmt' ), '@' ) ) . ')';
 ...
 * @param int $with_date
 *
 * @return bool|mixed|string False means no cloud service available to use on this site
 */
