...
trait TimerTrait {
	...
	function startT($fName = null) {
		...
		// Memory start
		$this->memoryProfiler[$fName]['sMem'][$callIndex]  = memory_get_usage(false); // actual usage
		$this->memoryProfiler[$fName]['sPeak'][$callIndex] = memory_get_peak_usage(true); // allocator peak
	}

	function endT($fName = null) {
		if (!$this->enabled || !$this->isEditing) {
			return;
		}
		...
		// --- Memory ---
		$endMem  = memory_get_usage(false);       // actual retained usage
